    ...
    public class TcpClientService
    {
        ...
        public async Task ConnectAsync(string ip = "10.10.20.114", int port = 12345)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"✅ 서버 연결 성공: {ip}:{port}");
            }
            catch (Exception ex)
            {
        ...
        public async Task<JObject> SendJsonToServer(JObject message)
        {
            ...
            try
            {
                ...
                await stream.FlushAsync();
                Console.WriteLine($"[전송 JSON]: {Encoding.UTF8.GetString(jsonBytes)}");
                Console.WriteLine($"[전송 길이]: {jsonBytes.Length}");
                Console.WriteLine($"[전송 바이트]: {BitConverter.ToString(jsonBytes)}");
            ...
            {
                Console.WriteLine("❌ 예외 발생: " + ex.ToString());
                return JObject.FromObject(new
                {
                    status = "error",
                    message = "전송 실패: " + ex.Message
                });
            }
        }
    }
