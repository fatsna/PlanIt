...
#include <winsock2.h>   // SOCKET 타입 정의
#include <ws2tcpip.h>   // 추가 네트워크 함수
...
public:
    ...
    // Python에게 JSON 요청을 보내고 응답을 받는 함수
    ...
    bool getUserCVByUid(const std::string& u_id, ResumeRow& out) {
        return db.getUserCVByUid(u_id, out);   // db는 객체이므로 점(.) 사용
    }

    DBClass db; // DBClass 멤버 변수
    DBClass* getDB() { return &db; }  // ★ 추가
